...
public class ArrayBag<T> implements BagInterface{
    ...
    @Override
    public BagInterface union(BagInterface bag) {
        int size = this.getCurrentSize() + bag.getCurrentSize();
        BagInterface unionBag = new ArrayBag(size);
        Object[] arrayBag1 = this.toArray();
        Object[] arrayBag2 = bag.toArray();
        
        for(int i=0 ; i<arrayBag1.length ; i++){
            unionBag.add(arrayBag1[i]);
        }
        for(int i=0 ; i<arrayBag2.length ; i++){
            unionBag.add(arrayBag2[i]);
        }
        return unionBag;
    }
